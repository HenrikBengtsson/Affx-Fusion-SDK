...
public class GenericData {
	 ...
	 */
	public AffymetrixGuidType getArrayFileIdentifier()
	{
		// Find the parent array file generic header
		AffymetrixGuidType arrayFileID=null;
		GenericDataHeader hdr = header.getGenericDataHdr().findParent("affymetrix-calvin-array");
		if (hdr!=null)
		{
	...
	{
		// Find the parent array file generic header
		AffymetrixGuidType arrayID=null;
		GenericDataHeader hdr = header.getGenericDataHdr().findParent("affymetrix-calvin-array");
		if (hdr!=null)
		{
			ParameterNameValue nvt = hdr.findNameValParam("affymetrix-array-id");
			if (nvt!=null)
			{
				arrayID = new AffymetrixGuidType(nvt.getValueAscii());
			}
		}
		return arrayID;
	}
	...
	/**
	 * Closes the file and releases any open input stream.
	 */
	public void close() {
    if (fileStream != null) try {
      fileStream.close();
    } catch (IOException ex) {
      // Should throw exception, but the API was written to throw no exception
      // before the fileStream.close() was added
    }
		fileStream = null;
	}
