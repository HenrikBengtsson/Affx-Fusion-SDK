...
public class FileInput {
	...
	public static short readInt16(FileInputStream fis) throws IOException {
		final byte[] b = new byte[DataSizes.SHORT_SIZE];
		fis.read(b);
	...
	public static int readInt32(FileInputStream fis) throws IOException {
		final byte[] b = new byte[DataSizes.INT_SIZE];
		fis.read(b);
	...
	public static float readFloat(FileInputStream fis) throws IOException {
		final byte[] b = new byte[DataSizes.FLOAT_SIZE];
		fis.read(b);
	...
	public static String readString16(FileInputStream fis, int len) throws IOException {
		final byte[] b = new byte[DataSizes.SHORT_SIZE * len];
		fis.read(b);
  ...
  /**
	...
	/**
	 * Reads an array of bytes of fixed size from a big endian file stream (memory map pointer).
	 *
	 * @param b
	 *          The input file stream.
	 * @param len
	 *          The length of the byte array.
	 * @return The bytes read from the file stream.
	 */
	public static byte[] readBytes(ByteBuffer b, int len) {
		final byte[] buf = new byte[len];
		b.get(buf);
		return buf;
	}
	...
	/**
	 * Reads an 8-bit string from a big endian file stream (memory map pointer)
   * as an array of bytes.  This retuns the same data as
   * {@link #readString8(java.nio.ByteBuffer)}, but avoids the expense of
   * creating a String from the bytes.
   * Using this method rather than {@link #readString8(java.nio.ByteBuffer)}
   ...
   * if you are going to interpret the data as a String.
	 ...
	 * @return The string read from the file stream.
	 */
	public static byte[] readString8AsBytes(ByteBuffer b) throws IOException {
		final int len = readInt32(b);
  ...
  /**
	...
	public static byte[] readBlob(FileInputStream fis) throws IOException {
		final int size = FileInput.readInt32(fis);
