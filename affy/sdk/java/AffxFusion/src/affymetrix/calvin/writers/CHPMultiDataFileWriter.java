...
public class CHPMultiDataFileWriter extends CHPFileWriterBase {
	 ...
	 *           the unsigned out of limits exception
	 */
	public void writeEntry(ProbeSetMultiDataCopyNumberVariationRegionData p) throws IOException, UnsignedOutOfLimitsException {
		writer.seekFromBeginPos(entryPos.get(currentDataType));
		dataSetWriter.write8Bit(p.getName(), maxProbeSetName);
		dataSetWriter.write(p.getSignal());
                dataSetWriter.write(p.getCall());
                dataSetWriter.write(p.getConfidenceScore());
		writeMetrics(p.getMetrics());
		entryPos.put(currentDataType, writer.getFilePos());
	}
	 ...
	 *           the unsigned out of limits exception
	 */
	public void writeEntry(DmetCopyNumberData p) throws IOException, UnsignedOutOfLimitsException {
		writer.seekFromBeginPos(entryPos.get(currentDataType));
		dataSetWriter.write8Bit(p.getName(), maxProbeSetName);
		dataSetWriter.write(p.getCall());
                dataSetWriter.write(p.getConfidence());
                dataSetWriter.write(p.getForce());
                dataSetWriter.write(p.getEstimate());
                dataSetWriter.write(p.getLower());
                dataSetWriter.write(p.getUpper());
		writeMetrics(p.getMetrics());
		entryPos.put(currentDataType, writer.getFilePos());
	}
	 ...
	 * 
	 * @param p
	 *          the multi data dmet bi-allelic data
	 * 
	 * @throws IOException
	 *           Signals that an I/O exception has occurred.
	 * @throws UnsignedOutOfLimitsException
	 *           the unsigned out of limits exception
	 */
	public void writeEntry(DmetBiAllelicData p) throws IOException, UnsignedOutOfLimitsException {
		writer.seekFromBeginPos(entryPos.get(currentDataType));
		dataSetWriter.write8Bit(p.getName(), maxProbeSetName);
		dataSetWriter.write(p.getCall());
                dataSetWriter.write(p.getConfidence());
                dataSetWriter.write(p.getForce());
                dataSetWriter.write(p.getSignalA());
                dataSetWriter.write(p.getSignalB());
                dataSetWriter.write(p.getContextA());
                dataSetWriter.write(p.getContextB());
		writeMetrics(p.getMetrics());
		entryPos.put(currentDataType, writer.getFilePos());
	}
	 ...
	 * 
	 * @param p
	 *          the multi data dmet multi-allelic data
	 * 
	 * @throws IOException
	 *           Signals that an I/O exception has occurred.
	 * @throws UnsignedOutOfLimitsException
	 *           the unsigned out of limits exception
	 */
	public void writeEntry(DmetMultiAllelicData p) throws IOException, UnsignedOutOfLimitsException {
		writer.seekFromBeginPos(entryPos.get(currentDataType));
		dataSetWriter.write8Bit(p.getName(), maxProbeSetName);
		dataSetWriter.write(p.getCall());
                dataSetWriter.write(p.getConfidence());
                dataSetWriter.write(p.getForce());
                dataSetWriter.write(p.getAlleleCount());
                dataSetWriter.write(p.getSignalA());
                dataSetWriter.write(p.getSignalB());
                dataSetWriter.write(p.getSignalC());
                dataSetWriter.write(p.getSignalD());
                dataSetWriter.write(p.getSignalE());
                dataSetWriter.write(p.getSignalF());
                dataSetWriter.write(p.getContextA());
                dataSetWriter.write(p.getContextB());
                dataSetWriter.write(p.getContextC());
                dataSetWriter.write(p.getContextD());
                dataSetWriter.write(p.getContextE());
                dataSetWriter.write(p.getContextF());
		writeMetrics(p.getMetrics());
		entryPos.put(currentDataType, writer.getFilePos());
	}
