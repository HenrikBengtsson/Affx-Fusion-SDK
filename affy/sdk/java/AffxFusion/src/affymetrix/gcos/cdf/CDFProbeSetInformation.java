...
public class CDFProbeSetInformation {
	...
	public CDFProbeGroupInformation getGroup(int index) {
		if (groups != null) {
            ...
            int groupSize = CDFProbeGroupInformation.PROBE_GROUP_SIZE;
            if (version == 2)
                groupSize = CDFProbeGroupInformation.PROBE_GROUP_SIZE_V2;
            else if (version > 2)
                groupSize = CDFProbeGroupInformation.PROBE_GROUP_SIZE_V3;
			...
			for (int i = 0; i < index; i++) {
				...
				groupOffset += (cells * probeSize);
			}
			group.setMap(xdaBuffer, groupOffset, setIndex, index, version);
			return group;
		}
		return null;
	}
	 ...
	 */
	public void setMap(MappedByteBuffer buf, long o, int index, int v) {
		xdaBuffer = buf;
		offset = (int)o;
        version = v;
